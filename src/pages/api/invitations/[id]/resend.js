// /pages/api/invitations/[id]/resend.js
import axios from "axios";

/**
 * API route for resending invitations
...
export default async function handler(req, res) {
  ...
  try {
    const response = await axios.post(
      `${NEXT_BACKEND_URL}invitations/${id}/resend`,
      {},
      {
    ...
    console.error(`Resend invitation API error for ID ${id}:`, error.response?.data || error.message);
    ...
    if (error.response?.status === 403) {
      return res.status(403).json({
        success: false,
        message: error.response.data.message || "You don't have permission to resend invitations"
      });
    }
