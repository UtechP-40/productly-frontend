// /pages/api/users/[userId]/role.js
import axios from "axios";

/**
 * API route for assigning roles to users
 * @route PATCH /api/users/[userId]/role - Assign role to user
...
export default async function handler(req, res) {
  ...
  try {
    const response = await axios.patch(
      `${NEXT_BACKEND_URL}users/${userId}/role`,
      { organizationId, role },
      {
    ...
    console.error("Assign role API error:", error.response?.data || error.message);
